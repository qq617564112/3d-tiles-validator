...
export function calcByteOffset(attributes: Attribute[]): number {
    let byteOffset = 0;
    for (const attrib of attributes) {
        const byteAlignment = attrib.byteAlignment;
...
/**
 * Inject partial attributes into a provided glTF asset.
 * @param gltf The glTF asset to modify
 * @param attributes An array of attributes to inject into the provided glTF 
 * asset
 */

export function addBinaryBuffers(gltf: Gltf, ...attributes: Attribute[]) {
